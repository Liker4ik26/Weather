...
internal class EitherWeatherAdapterFactory @Inject constructor() : CallAdapter.Factory() {
    override fun get(
        returnType: Type,
        ...
        val errorBodyConverter = retrofit.responseBodyConverter<WeatherErrorNetwork>(
            WeatherErrorNetwork::class.java,
            annotations
        )
    ...
    private class ResultCallAdapter<R, T>(
        ...
        private val errorBodyConverter: Converter<ResponseBody, WeatherErrorNetwork>
    ) :
    ...
    private class ResultCallWrapper<F, T>(
        ...
        private fun getReasonByCode(
            response: Response<T>,
            errorResponse: WeatherErrorNetwork
        ) = when (response.code()) {
